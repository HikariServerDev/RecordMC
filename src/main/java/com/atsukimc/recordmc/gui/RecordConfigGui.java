...
public class RecordConfigGui extends GuiConfigsBase {
    ...
    private String getWrappedHoverInfo(IConfigBase config) {
        ...
        for (String line : originalLines) {
            ...
            List<StringVisitable> wrappedLines = mc.textRenderer.getTextHandler().wrapLines(line, maxTooltipWidth, Style.EMPTY);
            for (StringVisitable wrapped : wrappedLines) {
                if (!first) {
                    sb.append("\n");
                }
    ...
    public List<ConfigOptionWrapper> getConfigs() {
        List<? extends fi.dy.masa.malilib.config.IConfigBase> options = switch (currentTab) {
            case GENERIC -> RecordConfigs.getGenericOptions();
            case HOTKEYS -> RecordConfigs.getHotkeyOptions();
        };
