...
public class ScreenRecorder {
    ...
    public synchronized void startRecording() {
        ...
        if (ffmpeg == null) {
            sendMessage(Text.translatable("recordmc.message.error_ffmpeg_not_found"));
            state = State.IDLE;
            return;
        }
        ...
        if (width <= 0 || height <= 0) {
            sendMessage(Text.literal("§c[RecordMC] 無効な画面解像度です: " + width + "x" + height));
            state = State.IDLE;
            return;
        }
        ...
        try {
            ...
            // 開始通知 & 効果音再生
            sendMessage(Text.translatable("recordmc.message.recording_started"));
            ...
            sendMessage(Text.translatable("recordmc.message.error_recording_failed", e.getMessage()));
            cleanupOnError();
            state = State.IDLE;
        }
    }
    ...
    public synchronized void stopRecording() {
        ...
        MinecraftClient client = MinecraftClient.getInstance();
        sendMessage(Text.translatable("recordmc.message.recording_stopping"));
        ...
        new Thread(() -> {
            try {
                ...
                if (success && finalOutputFile.exists()) {
                    ...
                    Text linkText = Text.literal(finalOutputFile.getName())
                            .formatted(Formatting.GREEN, Formatting.UNDERLINE)
                            .styled(style -> style.withClickEvent(new ClickEvent.OpenFile(finalOutputFile.getParentFile())));
                    Text message = Text.translatable(
                            copied ? "recordmc.message.recording_saved_and_copied" : "recordmc.message.recording_saved",
                            linkText
                    ).append(" ").append(Text.translatable("recordmc.message.recording_saved_click"));
                    sendMessage(message);
                } else {
                    LOGGER.error("Failed to create final video file");
                    sendMessage(Text.literal("§c[RecordMC] 動画の保存に失敗しました。"));
                }
            } catch (Exception e) {
